...
const Copyable = (props) => {
  ...
  const handleClick = async ({ partial }) => {
    const copyText = `@use "@nihatavci/graphics-components/scss/colours/${formatPartial(partial)}";`;
  ...
  return included ?
    ...
    : <div className={classes.importsnippet}>
        <SyntaxHighlighter language="scss" style={prismNord}>
          {`// global.scss \n@use "@nihatavci/graphics-components/scss/colours/${formatPartial(partial)}";`}
        </SyntaxHighlighter>
